...
const Projects = (props) => {
  ...
  const handleTaskLabelChange = (e, index) => {
    setSubtasks((pre) => {
      return pre.map((item) =>
        item?.id === index ? { ...item, label: e.target.value } : item
      );
    });
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      let task = formik?.values;
      task = {
        projectName: task?.name,
        dueDate: task?.dueDate,
        // projectTask: task?.taskIds,
        projectTask: subtasks.map((all) => all?.label),
        status: "doing",
        // resource: [resource],
        // customerName: "john",
      };
                ...
                return (
                  <FormGroup className="inputHead" key={task?.id}>
                    ...
                    <TextField
                      ...
                      onChange={(e) => handleTaskLabelChange(e, task?.id)}
                      value={task?.label}
                    />
                    {task?.label === "" && (
                      <p className="input-error">task is required</p>
                    )}
                  </FormGroup>
                );
              })}
